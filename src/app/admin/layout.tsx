    ...
    <div className="min-h-screen bg-[#000000] text-gray-200 flex flex-col md:flex-row font-sans">
      ...
      <header className={`${isChatPage ? "hidden" : ""} md:hidden w-full bg-[#0a0a0a]/95 backdrop-blur-xl border-b border-white/5 px-5 py-4 sticky top-0 z-[99] flex items-center justify-between shadow-sm`}>
        ...
        <div className="flex items-center gap-2.5 shrink-0">
          <Link href="/admin/pengaturan" title="Pengaturan Akun" className="w-8 h-8 rounded-full bg-[#2b5cd5]/10 text-[#b3c5ff] flex items-center justify-center font-black text-[10px] border border-[#2b5cd5]/20 shadow-inner active:scale-90 transition-transform">HR</Link>
          ...
          {navItems.filter((item) => item.href !== "/admin/pengaturan").map((item) => {
            ...
            return (
              <Link key={item.name} href={item.href} className={`flex flex-col items-center gap-1.5 transition-all p-2 rounded-xl ${isActive ? "text-white bg-white/10" : "text-gray-500 hover:text-gray-300"}`}>
                ...
                <span className="text-[9px] font-bold">{(item as any).short || item.name}</span>
              </Link>
            );
          })}
        </div>
