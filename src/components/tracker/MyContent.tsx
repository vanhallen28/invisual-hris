  ...
  return (
    <div className="mt-8">
      <div className="flex items-center gap-2 mb-2.5">
        ...
        <span className="text-xs font-bold text-zinc-400 uppercase tracking-wider">Konten Saya ({posts.length})</span>
      </div>

      <div className="flex flex-col gap-1.5">
        {posts.map((p) => (
          <button key={p.id} onClick={() => setOpen(p)}
            className="group/c w-full flex items-center gap-3 bg-[#20222b] hover:bg-[#262934] border border-zinc-800/80 hover:border-zinc-700 rounded-lg px-4 py-3 text-left transition-colors">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                ...
                <span className="text-[10px] text-zinc-500">{p.content_type}</span>
              </div>
            </div>
            <span className={`text-[10px] px-2 py-0.5 rounded-full font-semibold text-white shrink-0 ${statusColor(p.status)}`}>{p.status}</span>
            <span className="hidden sm:flex items-center gap-1 text-[11px] text-zinc-500 shrink-0 w-20 justify-end"><CalendarDays size={11} />{fmt(p.publish_at)}</span>
            <ChevronRight size={14} className="text-zinc-600 group-hover/c:text-zinc-300 shrink-0 transition-colors" />
          </button>
        ))}
      </div>
