...
export default function TableCell({ type, item, group, col }: any) {
  ...
  const cellBorder = (isSub ? 'border-r border-zinc-700/50' : 'border-r border-zinc-800/40') + ' dwt-cell-in';
  
  const activePopupPos = 'left-1/2 -translate-x-1/2'; 

  const getLabelColor = (field: string, value: string) => { const match = labels[field]?.find((l: any) => l.text === value); return match ? `${match.color} text-white shadow-sm` : 'bg-zinc-800/40 text-zinc-500 border border-zinc-800'; };
  ...
  if (col.type === 'checkbox') {
    return (
      <div className={`${cellBorder} p-[2px] flex items-center justify-center min-w-0`}>
        <input type="checkbox" checked={!!item[col.id]} onChange={(e) => triggerUpdate(col.id, e.target.checked)} onClick={e => e.stopPropagation()} onMouseDown={e => e.stopPropagation()} className="w-4 h-4 rounded bg-zinc-950 border-zinc-700 text-blue-500 cursor-pointer shadow-inner focus:ring-0" />
      </div>
    );
  }
  ...
  if (col.type === 'date') {
    ...
    let dueCls = 'text-zinc-300 group-hover/date:text-white';
    if (dv) {
      ...
      <div className={`relative ${cellBorder} p-1 ${isDrop ? 'z-[100]' : ''}`}>
        ...
        {isDrop && (
          <>
            ...
            <div className={`absolute top-full mt-1.5 ${activePopupPos} bg-[#2a2c38] border border-zinc-600 shadow-2xl rounded-xl z-50 p-2 w-56 flex flex-col text-left animate-in fade-in zoom-in-95`} onClick={e=>e.stopPropagation()}>
    ...
    return (
      <div className={`relative ${cellBorder} p-[2px] ${isDrop ? 'z-[100]' : ''} min-w-0 flex items-center justify-center`}>
        ...
        {isDrop && (
           <>
             <div className="fixed inset-0 z-40" onClick={(e) => { e.stopPropagation(); setOpenDropdown(null); }}></div>
             <div className={`absolute top-full mt-1.5 ${activePopupPos} bg-[#2a2c38] border border-zinc-600 shadow-2xl rounded-xl z-50 p-1.5 w-56 flex flex-col animate-in fade-in zoom-in-95`} onClick={e=>e.stopPropagation()}>
               <div className="text-[10px] text-zinc-400 font-bold uppercase tracking-wider px-2 py-1 mb-1">{col.label}</div>
               {!isMulti && <button onClick={() => { triggerUpdate(col.id, ''); setOpenDropdown(null); }} className="text-left text-xs px-3 py-2 hover:bg-zinc-700 rounded-lg text-zinc-400 w-full mb-1 transition-colors shrink-0">- Reset</button>}
               <div className="max-h-48 overflow-y-auto flex flex-col gap-0.5 pr-1 custom-scrollbar mb-1.5">
                 {(labels[col.id] || []).map((l: any) => {
                   ...
                   return (
                     <div key={l.id}>
                       <div className="flex items-center justify-between text-xs px-3 py-2 hover:bg-zinc-700 rounded-lg text-zinc-200 w-full group/menuitem cursor-pointer transition-colors" onClick={() => { if(isMulti) { triggerUpdate(col.id, hasMulti ? item[col.id].filter((t:any)=>t!==l.text) : [...(item[col.id]||[]), l.text]) } else { triggerUpdate(col.id, l.text); setOpenDropdown(null); } }}>
                         ...
                         <div className="flex items-center gap-2 shrink-0">
                           ...
                           <button onClick={(e) => { e.stopPropagation(); triggerConfirm('Hapus Label', `Hapus opsi label "${l.text}"?`, () => handleDeleteLabel(col.id, l.id)); }} className="opacity-0 group-hover/menuitem:opacity-100 p-0.5 text-zinc-500 hover:text-red-400 transition-opacity shrink-0"><Trash2 size={13}/></button>
                         </div>
                       </div>
                       {colorPickerFor === l.id && (
               ...
               <div className="px-1 pt-2 border-t border-zinc-700/50 shrink-0">
                 ...
                 <input autoComplete="off" spellCheck="false" autoFocus value={newLabelText} onChange={e=>setNewLabelText(e.target.value)} onKeyDown={e=>e.key==='Enter'&&handleAddNewLabel(col.id, isMulti)} placeholder="+ Opsi baru (pilih warna di atas)" className="bg-zinc-950 border border-zinc-700 focus:border-blue-500 transition-colors rounded-md px-3 py-1.5 text-[11px] w-full text-white outline-none shadow-inner min-w-0"/>
               </div>
             </div>
           </>
        )}
  ...
  if (col.type === 'timeline') {
    ...
    return (
      <div className={`${cellBorder} p-1 relative flex items-center justify-center ${isDrop ? 'z-[100]' : ''}`}>
        <div onClick={() => { setTempTimeline(tData); setOpenDropdown({ type, groupId: group.id, itemId: actualItemId, subItemId: actualSubItemId, field: col.id }); }} className="relative w-full h-full min-h-[24px] bg-zinc-800/30 hover:bg-zinc-800/60 rounded-full flex items-center justify-center cursor-pointer overflow-hidden border border-zinc-800 transition-colors">
        ...
        {isDrop && (
          <>
            <div className="fixed inset-0 z-40" onClick={(e) => { e.stopPropagation(); setOpenDropdown(null); }}></div>
            <div className={`absolute top-full mt-1.5 ${activePopupPos} bg-[#2a2c38] border border-zinc-600 shadow-2xl rounded-xl z-50 p-4 flex flex-col gap-3 animate-in fade-in zoom-in-95`} onClick={e=>e.stopPropagation()}>
              <div><label className="text-[10px] font-bold text-zinc-400 block mb-1.5 uppercase tracking-wider">Start Date</label><input type="date" value={tempTimeline?.start || ''} onChange={e=>setTempTimeline({...tempTimeline, start:e.target.value})} className="w-full bg-zinc-950 border border-zinc-700 rounded-md px-2.5 py-1.5 text-xs text-white [color-scheme:dark] outline-none shadow-inner" /></div>
              <div><label className="text-[10px] font-bold text-zinc-400 block mb-1.5 uppercase tracking-wider">End Date</label><input type="date" value={tempTimeline?.end || ''} onChange={e=>setTempTimeline({...tempTimeline, end:e.target.value})} className="w-full bg-zinc-950 border border-zinc-700 rounded-md px-2.5 py-1.5 text-xs text-white [color-scheme:dark] outline-none shadow-inner" /></div>
              <div className="flex justify-between items-center border-t border-zinc-700/50 pt-3 mt-1"><button onClick={() => { triggerUpdate(col.id, null); setOpenDropdown(null); }} className="text-xs text-red-400 font-bold hover:text-red-300 transition-colors">Clear</button><button onClick={() => { triggerUpdate(col.id, tempTimeline); setOpenDropdown(null); }} className="text-xs bg-blue-600 hover:bg-blue-500 px-4 py-1.5 rounded-md text-white font-bold transition-colors shadow-md">Set Date</button></div>
            </div>
          </>
        )}
      </div>
    ...
    <div className={`${cellBorder} px-2 py-1 flex items-center justify-center min-w-0 group/textcell`}>
      <div className="w-full min-w-0 flex items-center justify-center">
        <InlineEdit
           ...
           textClassName={`transition-colors truncate ${col.type === 'number' ? 'text-right' : 'text-center'} ${col.id.includes('link') && safeValue ? 'text-blue-400 hover:underline' : 'text-zinc-300 group-hover/textcell:text-white'}`}
