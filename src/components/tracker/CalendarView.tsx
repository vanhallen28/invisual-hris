    ...
    <div className="bg-kartu border border-white/10 rounded-xl overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-white/10">
        <div className="flex items-center gap-3 min-w-0">
          <h2 className="text-base font-bold text-gray-100 whitespace-nowrap">{MONTHS[month]} {year}</h2>
          <span className="text-[11px] text-gray-500 truncate">{totalEvents > 0 ? `${totalEvents} item terjadwal` : 'Belum ada item bertanggal — tambah kolom Date / Timeline'}</span>
        </div>
        <div className="flex items-center gap-1 shrink-0">
          <button onClick={() => setViewDate(new Date(year, month - 1, 1))} className="p-1.5 rounded-md hover:bg-kartu-hover text-gray-400 hover:text-white transition-colors"><ChevronLeft size={16} /></button>
          <button onClick={() => { const d = new Date(); setViewDate(new Date(d.getFullYear(), d.getMonth(), 1)); }} className="px-2.5 py-1 rounded-md hover:bg-kartu-hover text-[11px] font-semibold text-gray-300 transition-colors">Hari ini</button>
          <button onClick={() => setViewDate(new Date(year, month + 1, 1))} className="p-1.5 rounded-md hover:bg-kartu-hover text-gray-400 hover:text-white transition-colors"><ChevronRight size={16} /></button>
        </div>
      </div>

      {/* Label hari */}
      <div className="grid grid-cols-7 border-b border-white/10">
        {DAYS.map(d => <div key={d} className="px-2 py-2 text-[10px] font-bold uppercase tracking-wider text-gray-500 text-center">{d}</div>)}
      </div>

      {/* Grid tanggal */}
      <div className="grid grid-cols-7">
        {cells.map((day, idx) => {
          if (day === null) return <div key={idx} className="border-r border-b border-white/10 bg-kartu/20 min-h-[96px]"></div>;
          const key = `${year}-${String(month + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}`;
          const dayEvents = events[key] || [];
          const isToday = key === todayStr;
          return (
            <div key={idx} className="border-r border-b border-white/10 p-1.5 min-h-[96px] flex flex-col gap-1 hover:bg-white/5 transition-colors">
