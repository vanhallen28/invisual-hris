...
function BoardContent({ onMenuClick }: any) {
  ...
  return (
    <div className="flex-1 flex flex-col bg-kartu overflow-hidden min-w-0">
      <div className="h-12 border-b border-white/10 bg-input flex items-center gap-2 px-3 shrink-0 z-30">
        <Link href={backHref} title="Kembali ke Portal HRIS" className="flex items-center gap-1.5 text-xs font-bold text-white bg-primer-terang hover:bg-primer px-3 py-1.5 rounded-lg transition-all shrink-0 shadow-[0_0_14px_rgba(43,92,213,0.45)] hover:shadow-[0_0_18px_rgba(43,92,213,0.6)]"><ChevronLeft size={15} /> Portal</Link>
        <button onClick={onMenuClick} className="md:hidden p-1.5 text-gray-400 hover:text-white rounded transition-colors shrink-0"><Menu size={18} /></button>
        <div className="w-px h-5 bg-kartu-hover mx-1 hidden sm:block" />
        <div className="flex items-center gap-2 text-xs font-semibold min-w-0">
          {activeBoardPath ? (
            <>
              <span className="text-gray-500 truncate hidden sm:inline">{activeBoardPath.year}</span>
              <span className="text-gray-600 hidden sm:inline">›</span>
              <span className="text-gray-500 truncate hidden sm:inline">{activeBoardPath.month}</span>
              <span className="text-gray-600 hidden sm:inline">›</span>
              <span className="text-gray-100 font-bold truncate">{activeBoardPath.board}</span>
            </>
          ) : (
            <span className="text-gray-200 font-bold uppercase tracking-widest">Daily Work Tracker</span>
          )}
        </div>
      </div>
      ...
      {!activeBoardId ? (
        <div className="flex-1 flex flex-col items-center justify-center text-gray-500 p-6">
          <div className="bg-kartu/30 p-10 rounded-3xl flex flex-col items-center border border-white/10 shadow-2xl">
            <LayoutGrid size={64} className="mb-6 text-blue-500/20" />
            <h2 className="text-xl font-bold text-gray-300 mb-2">Belum Ada Project Terpilih</h2>
            <p className="text-sm text-gray-500 max-w-sm text-center">Pilih atau buat project baru di menu Sidebar sebelah kiri.</p>
          </div>
        </div>
      ) : (
            ...
            <div className="flex flex-col gap-4 mb-6 border-b border-white/10 pb-0 shrink-0">
              ...
              <div className="flex items-center gap-3 flex-wrap">
                ...
                {canContentHub && <button
                  ...
                  className={`flex items-center gap-1.5 text-xs font-bold px-3.5 py-2 rounded-lg transition-all shrink-0 ${contentMode ? 'bg-primer-terang text-white shadow-[0_0_14px_rgba(43,92,213,0.4)]' : 'bg-white/5 text-gray-400 hover:text-white hover:bg-kartu-hover'}`}
                >
...
export default function ManagerBoard() {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex bg-kartu text-gray-100 font-sans overflow-hidden animate-in fade-in duration-200">
