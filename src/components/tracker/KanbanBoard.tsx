...
export default function KanbanBoard() {
  ...
  const formatDue = (d?: string) => {
    ...
    const cls = diff < 0 ? 'text-red-400 bg-red-500/10' : diff <= 3 ? 'text-amber-400 bg-amber-500/10' : 'text-zinc-400 bg-zinc-800/60';
    return { label, cls };
  };
  ...
  if (!statusKey) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center text-zinc-500 mt-4 py-16">
        ...
        <h3 className="text-zinc-300 font-bold mb-1">Kanban needs a Status column</h3>
        <p className="text-sm text-zinc-500 max-w-sm text-center">Go to Main Table, add a <span className="text-zinc-300 font-semibold">Status</span> column, then drag cards between its values here.</p>
      </div>
    );
  }

  const statusColumns = [...(labels[statusKey] || []), { id: 'empty', text: '', color: 'bg-zinc-800' }];
  ...
  return (
    <div className="flex gap-6 overflow-x-auto pb-8 flex-1 items-start mt-4">
      {statusColumns.map((col: any) => {
        ...
        return (
          <div key={col.id} onDragOver={e => handleDragOver(e, col.text)} onDrop={e => handleDrop(e, col.text)} className={`w-[320px] shrink-0 flex flex-col rounded-xl border bg-[#1e202a] max-h-full ${dragOverColumn === col.text ? 'border-blue-500 bg-[#252836]' : 'border-zinc-800/60'}`}>
            <div className="p-4 border-b border-zinc-800/60 bg-[#1e202a] rounded-t-xl sticky top-0"><div className="flex justify-between mb-2"><h3 className={`text-xs font-bold text-white px-2.5 py-1 rounded ${col.color || 'bg-zinc-800'}`}>{col.text || 'No Status'}</h3><span className="text-xs text-zinc-500 font-bold bg-zinc-800/50 px-2 py-0.5 rounded">{items.length}</span></div></div>
            <div className="p-3 flex flex-col gap-3 overflow-y-auto">
              {items.map((item: any) => {
                 ...
                 return (
                 <div key={item.id} draggable onDragStart={e => handleDragStart(e, item.groupId, item.id)} onDragEnd={() => setDraggedItem(null)} onClick={() => setDetailItem({ groupId: item.groupId, itemId: item.id })} className="bg-[#2a2c38] rounded-lg p-4 border border-zinc-700/60 cursor-pointer hover:border-blue-500/60 hover:bg-[#30323e] transition-all" style={{ borderLeftColor: item.groupColor, borderLeftWidth: '3px' }}>
