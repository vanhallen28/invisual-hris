...
const TBtn = ({ onDo, title, children }: any) => (
  <button onMouseDown={(e) => { e.preventDefault(); onDo(); }} title={title} className="p-1.5 hover:bg-kartu-hover rounded text-gray-400 hover:text-white transition-colors">{children}</button>
);
...
export default function DocEditor() {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm flex items-center justify-center p-3 sm:p-6" onMouseDown={(e) => { if (e.target === e.currentTarget) close(); }}>
      <div className="w-full max-w-3xl h-[90vh] bg-kartu text-gray-100 rounded-xl shadow-2xl flex flex-col overflow-hidden border border-white/10">
        <div className="flex items-center justify-between px-4 h-12 border-b border-white/10 bg-kartu-hover shrink-0">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <span className="text-sm font-semibold text-gray-200 truncate">{docEditorTarget.title || 'Dokumen'}</span>
          </div>
          <div className="flex items-center gap-3 shrink-0">
            <span className="text-[11px] text-gray-500">{uploading ? 'Mengunggah…' : saved ? 'Tersimpan' : 'Menyimpan…'}</span>
            <button onClick={close} className="p-1 text-gray-400 hover:text-white rounded transition-colors"><X size={18} /></button>
          </div>
        </div>

        <div className="flex items-center gap-0.5 px-3 py-1.5 border-b border-white/10 bg-kartu shrink-0 flex-wrap">
          ...
          <div className="w-px h-5 bg-kartu-hover mx-1" />
          ...
          <button onMouseDown={(e) => { e.preventDefault(); cmd('formatBlock', '<p>'); }} title="Paragraf" className="px-2 py-1 hover:bg-kartu-hover rounded text-gray-400 hover:text-white text-xs font-bold transition-colors">P</button>
          <div className="w-px h-5 bg-kartu-hover mx-1" />
        ...
        <div className="border-t border-white/10 bg-kartu-hover shrink-0">
          <button onClick={() => setShowComments((s) => !s)} className="w-full flex items-center justify-between px-4 py-2 text-xs text-gray-400 hover:text-white transition-colors">
          ...
          {showComments && (
            <div className="border-t border-white/10 px-4 py-3 max-h-52 overflow-y-auto flex flex-col gap-3">
              {comments.length === 0 && <div className="text-[11px] text-gray-500">Belum ada komentar.</div>}
              {comments.map((c) => (
                <div key={c.id} className="flex gap-2.5">
                  <div className="w-6 h-6 rounded-full bg-primer-terang flex items-center justify-center text-[9px] font-bold text-white shrink-0">{authorInit(c.author_id)}</div>
                  <div className="min-w-0">
                    <div className="text-[11px] text-gray-400"><b className="text-gray-200">{authorName(c.author_id)}</b><span className="text-gray-600 ml-1.5">{timeShort(c.created_at)}</span></div>
                    <div className="text-[13px] text-gray-200 whitespace-pre-wrap break-words">{c.text}</div>
                  </div>
                </div>
              ))}
            </div>
          )}
          <div className="flex gap-2 px-4 py-2.5 border-t border-white/10">
            <input value={commentText} onChange={(e) => setCommentText(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') postComment(); }} placeholder="Tulis komentar…" className="flex-1 bg-latar border border-white/10 focus:border-blue-500 rounded-lg px-3 py-1.5 text-[13px] text-white outline-none transition-colors min-w-0" />
