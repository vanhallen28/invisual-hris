...
export default function TaskChat({ itemId, itemName }: { itemId: string; itemName?: string }) {
  ...
  const authorColor = (id: string) => { const c = member(id)?.color; return c && String(c).startsWith('bg-') ? c : 'bg-primer-terang'; };
  ...
  const renderBody = (t: string) => {
    if (isUrl(t)) {
      const url = t.trim();
      if (isImg(url)) return <a href={url} target="_blank" rel="noopener noreferrer"><img src={url} alt="" className="max-w-[220px] max-h-[220px] rounded-lg border border-white/10 mt-1" /></a>;
    ...
    return <span className="text-gray-300 leading-relaxed break-words whitespace-pre-wrap">{t}</span>;
  };

  const messages = (
    <div className="flex flex-col gap-3.5">
      {updates.length === 0 && <p className="text-xs text-gray-600 italic">Belum ada pesan. Mulai diskusi dengan tim.</p>}
      {updates.map((upd: any) => (
        <div key={upd.id} className="text-xs">
          <div className="flex items-baseline gap-2 flex-wrap">
            ...
            <span className="text-[10px] text-gray-500">{timeShort(upd.created_at)}</span>
          </div>
  ...
  const composer = (
    <form onSubmit={post} className="flex items-center gap-1.5 bg-kartu border border-white/10 p-1.5 rounded-xl focus-within:border-blue-500 transition-colors">
      <button type="button" onClick={() => fileRef.current?.click()} disabled={uploading} title="Lampirkan file / gambar" className="text-gray-400 hover:text-white p-1.5 rounded-lg hover:bg-kartu-hover transition-colors shrink-0 disabled:opacity-50"><Paperclip size={15} /></button>
      <input value={text} onChange={(e) => setText(e.target.value)} placeholder={uploading ? 'Mengunggah…' : 'Tulis pesan ke tim...'} className="w-full bg-transparent text-xs text-gray-200 px-1 outline-none" />
      <button type="submit" disabled={!text.trim()} className="bg-blue-600 disabled:bg-kartu-hover disabled:text-gray-600 text-white p-1.5 rounded-lg transition-colors shrink-0"><Send size={14} /></button>
      ...
      <div className="fixed inset-0 z-[110] bg-black/60 backdrop-blur-sm flex items-center justify-center p-3 sm:p-6" onMouseDown={(e) => { if (e.target === e.currentTarget) setExpanded(false); }}>
        <div className="w-full max-w-2xl h-[88vh] bg-input rounded-xl shadow-2xl flex flex-col overflow-hidden border border-white/10">
          <div className="flex items-center justify-between px-4 h-12 border-b border-white/10 bg-kartu-hover shrink-0">
            <div className="flex items-center gap-2 min-w-0"><MessageSquare size={15} className="text-blue-400 shrink-0" /><span className="text-sm font-semibold text-gray-200 truncate">Diskusi: {itemName || 'Tugas'}</span></div>
            <button onClick={() => setExpanded(false)} title="Perkecil" className="flex items-center gap-1.5 text-xs text-gray-400 hover:text-white px-2 py-1 rounded-lg hover:bg-kartu-hover transition-colors"><Minimize2 size={15} /> Perkecil</button>
          </div>
  ...
  return (
    <div className="border border-white/10 rounded-xl bg-input overflow-hidden">
      <div className="flex items-center justify-between px-3 py-2 border-b border-white/10">
        <div className="text-[11px] font-bold text-gray-400 uppercase tracking-wider flex items-center gap-2"><Send size={12} /> Diskusi Tugas (Chat)</div>
        <button onClick={() => setExpanded(true)} title="Perbesar" className="p-1 text-gray-500 hover:text-white rounded transition-colors"><Maximize2 size={14} /></button>
      </div>
      <div ref={scrollRef} className="h-64 overflow-y-auto overscroll-contain px-3 py-3">{messages}</div>
      <div className="p-2.5 border-t border-white/10 bg-input">{composer}</div>
    </div>
  );
}
